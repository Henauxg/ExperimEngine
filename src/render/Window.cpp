...
Window::Window(int width, int height, const char* title)
{
	...
	sdlWindow_
		= SDL_CreateWindow(title, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width,
...
vk::SurfaceKHR Window::createSurface(vk::Instance vkInstance) const
{
	...
	ASSERT_RESULT(SDL_Vulkan_CreateSurface(sdlWindow_, vkInstance, &surface),
...
std::vector<const char*> Window::getRequiredVkExtensions() const
{
	...
	ASSERT_RESULT(
		...
		"Failed to get the names of the Vulkan extensions required by the SDL window");
